...
- Default and outline buttons have the brand's pressable look: a solid edge and a hard 3px offset shadow the button sinks into when pressed. The yellow button's edge is `--primary-edge` (black in light mode, a shaded gold in dark mode so it reads as the side of a yellow key); outline buttons use `--outline-edge` (black in light mode, a muted gray in dark mode, where a cream edge glares). The same edge marks the active sidebar item, the user avatar, and toasts: toasts are brand blue (red for errors), from the `--toast`, `--toast-foreground`, and `--toast-edge` tokens.
