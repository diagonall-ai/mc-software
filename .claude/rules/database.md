...
- `app/db/auth.schema.ts` is generated by the Better Auth CLI: never edit it. After changing Better Auth plugins, regenerate it with `NODE_OPTIONS=--unhandled-rejections=warn pnpm dlx auth@latest generate --config app/lib/auth-server.ts --output app/db/auth.schema.ts --yes` (without `NODE_OPTIONS`, the CLI crashes on the OAuth provider's startup check).
